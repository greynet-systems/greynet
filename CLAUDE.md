...
Plug-and-play антицензурный прокси-узел. Пользователь втыкает коробку в любой порт
домашнего роутера, она автонастраивается и раздаёт защищённый доступ через mesh-сеть.
...
7. Устройства пользователя подключаются через Tailscale → прокси на коробке → zapret2 обходит DPI на выходе
